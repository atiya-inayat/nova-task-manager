...
export default function HowItWorksSection() {
  ...
  return (
    <section className="relative py-20 md:py-28 overflow-hidden bg-black text-white px-6">
      {/* Background blobs hidden on very small screens to save performance */}
      <div className="absolute top-20 left-10 w-48 md:w-72 h-48 md:h-72 bg-slate-500/10 blur-3xl rounded-full" />
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.number} className="relative group">
                <div className="h-full rounded-2xl p-8 border border-slate-800 bg-slate-900/50 backdrop-blur-sm shadow-xl hover:bg-slate-900 transition-all">
                  <div className="absolute top-4 right-4 text-4xl font-bold text-white/5">
                    {step.number}
                  </div>
                  <div className="mb-6 inline-flex p-3 rounded-xl bg-indigo-500/10 text-indigo-400">
                ...
                {/* Responsive Arrow: Hidden on mobile, flex on MD screens */}
                {index < steps.length - 1 && (
                  <div className="hidden md:flex absolute top-1/2 -right-6 z-10 text-slate-700">
